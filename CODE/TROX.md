...
* 不涉及收发指令
  * 屏保：通过触摸监控以及定时组件，一定时间没有触碰则进入屏保，待触碰后返回原页面
  * 按键声音：通过按钮组件以及定时组件，检测一定时间内是否两次触碰按钮
  * 页面跳转：通过按钮实现(故障、设置、英文)
  * 通讯状态：记录通讯是否异常
* 设计收发指令(所有发送指令都有返回验证，若发送50次未有接收返回值，则超时退出，反馈通信错误)
  * 正常模式(上电默认正常模式，从关阀跳转至开阀、退出紧急模式、退出夜间模式都会(要)进入正常模式)
  * 紧急模式：按下切换状态
  * 夜间模式：按下切换状态
  * 关阀：(可从正常模式、紧急模式、夜间模式关阀)
  * 开阀：从关阀切换，进入正常模式
  * 参数读取：发送读取指令并更新
  * 故障状态：发送状态读取指令并更新
  * 警报触发：发送警报指令

  通过TTL-485监测控制板与TROX板之间的通信，确定各指令
